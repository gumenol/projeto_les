...
<body>
		...
		List<EntidadeDominio> produtos = ResultadoProdutos.getEntidades();
			...
			String redirectURL = "Produtos?operacao=CONSULTAR";
	...
	for(EntidadeDominio ent : produtos){
		prd = (Produto)ent;
		if(prd.getId()==id){
			categoriaProduto = prd.getCategoria().getNome();
			marcaProduto = prd.getMarca_produto();
			break;
		}
	}
	%>
			...
			<div style="text-align: center">
				<form id="form" action="Produtos" method="POST" class="form-horizontal">
					<section class="panel">
						...
						<div class="col-sm-1">
								<a href="Produtos?operacao=CONSULTAR" style="background-color:grey; color:white" class="btn btn-default">Voltar para a listagem</a>
							</div>
						...
						<div class="panel-body" style="padding-right:100px">
							...
							<label class="col-sm-3 control-label">Id Produto</label>
								...
								for(EntidadeDominio ent : produtos){
									prd = (Produto)ent;
								if(prd.getId()==id){
									sb.append("<input type='hidden' id='id-produto'name='id-produto' value='"+prd.getId());
									sb.append("'>");
							...
							<div class="form-group">
							<label class="col-sm-3 control-label">Quantidade no estoque</label>
								<div class="col-sm-1">
								<%
								sb = new StringBuilder();
								for(EntidadeDominio ent : produtos){
									prd = (Produto)ent;
								if(prd.getId()==id){
									sb.append("<input type='text' id='qtd-estoque'name='qtd-estoque' value='"+prd.getQtd_estoque());
							...
							<div class="form-group">
								...
								for(EntidadeDominio ent : produtos){
									prd = (Produto)ent;
								if(prd.getId()==id){
									sb.append("<input id='nome' type='text' name='nome-produto' value='"+prd.getNome().trim());
									sb.append(" 'class='col-sm-3 form-control' ");
							...
							<div class="form-group" style="margin-bottom:5px">
								...
								for(Categoria cat : itens.getCategoria()){
											...
											if(cat.getNome().equals(categoriaProduto)){
							...
							<div class="form-group" style="margin-bottom:4px">
								...
								for(Marca m : itens.getMarcas()){
											...
											if(m.getNome_marca().equals(marcaProduto)){
												sb.append("<option value='"+ m.getId()+"' selected>");
												sb.append(marcaProduto+"</option>");
											}
											else{
							...
							<div class="form-group">
								...
								for(EntidadeDominio ent : produtos){
									prd = (Produto)ent;
								if(prd.getId()==id){
									sb.append("<input id='img' type='text' name='img-produto' value='"+prd.getImg_produto());
									sb.append(" 'class='col-sm-3 form-control' ");
									sb.append("required oninvalid='this.setCustomValidity");
									sb.append("('O campo nome é obrigatório!') oninput='setCustomValidity('')/>");
									}
							...
							<div class="form-group">
								...
								if(prd.getId()==id){
									...
									sb.append("required oninvalid='this.setCustomValidity");
									sb.append("('O campo descricao é obrigatório!') oninput='setCustomValidity('')>"+ prd.getDescricao_produto() + "</textarea>");
									}
								}
								out.print(sb.toString());
								%>
